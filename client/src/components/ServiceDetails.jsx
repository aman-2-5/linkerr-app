...
const ServiceDetails = () => {
  ...
  useEffect(() => {
    const fetchService = async () => {
      try {
        const res = await axios.get(`https://linkerr-api.onrender.com/api/services/${id}`);
        setService(res.data);
        setLoading(false);
      } catch (err) {
        console.error(err);
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="flex items-center gap-3 py-4 border-y border-slate-100">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">{service.seller?.headline || "Freelancer on Linkerr"}</p>
            </div>
          </div>
